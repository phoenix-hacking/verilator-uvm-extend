// -*- Verilog -*-
// DESCRIPTION: Verilator: AXI register model, explicit predictor, and built-in RAL sequences
//
// This file ONLY is placed under the Creative Commons Public Domain.
// SPDX-FileCopyrightText: 2026 Wilson Snyder
// SPDX-License-Identifier: CC0-1.0

// Included inside module t after axi_sequence. Oracles follow the pinned
...
class axi_reg_adapter extends uvm_reg_adapter;
  ...
  virtual function uvm_sequence_item reg2bus(const ref uvm_reg_bus_op rw);
    axi_item item = axi_item::type_id::create("register_transfer");
    if (rw.addr >= 64 || rw.addr % 4 != 0 || rw.n_bits > 32)
      `uvm_fatal("AXI_RAL_ADAPTER", "register operation exceeds the AXI register map")
    item.wr = rw.kind == UVM_WRITE;
    item.rd = rw.kind == UVM_READ;
    item.addr = 8'(rw.addr);
    item.read_addr = 8'(rw.addr);
    item.data = 32'(rw.data);
    item.strb = item.wr ? 4'(rw.byte_en) : 4'b0;
    item.aw_delay = rw.addr[2] ? 4 : 0;
    item.w_delay = rw.addr[2] ? 0 : 4;
    item.ar_delay = int'(rw.addr[2]);
    item.b_delay = 1;
    item.r_delay = 2;
    return item;
  endfunction
  ...
  virtual function void bus2reg(uvm_sequence_item bus_item, ref uvm_reg_bus_op rw);
    ...
    if (!$cast(item, bus_item)) begin
      `uvm_fatal("AXI_RAL_ADAPTER", "adapter received an incompatible bus item")
      return;
    end
    if (item.wr == item.rd)
      `uvm_fatal("AXI_RAL_ADAPTER", "register response must contain exactly one direction")
    rw.addr = uvm_reg_addr_t'(item.addr);
    rw.kind = item.wr ? UVM_WRITE : UVM_READ;
    rw.data = uvm_reg_data_t'(item.data);
    rw.n_bits = 32;
    rw.byte_en = item.wr ? uvm_reg_byte_en_t'(item.strb) : '1;
    rw.status = item.response != 0 || item.cancel ? UVM_NOT_OK : UVM_IS_OK;
  endfunction
endclass

class axi_ral_backdoor extends uvm_reg_backdoor;
  `uvm_object_utils(axi_ral_backdoor)
  ...
  function new(string name = "axi_ral_backdoor");
    super.new(name);
  endfunction

  virtual function void read_func(uvm_reg_item rw);
    if (index >= 16) `uvm_fatal("AXI_RAL_BACKDOOR", "backdoor register index is outside the DUT")
  ...
  virtual task write(uvm_reg_item rw);
    ...
    if (index >= 16 || active_scoreboard == null || passive_scoreboard == null)
      `uvm_fatal("AXI_RAL_BACKDOOR", "backdoor write is not configured")
...
class axi_word_reg extends uvm_reg;
  `uvm_object_utils(axi_word_reg)

  uvm_reg_field octets[4];

  function new(string name = "axi_word_reg");
...
class axi_reg_block extends uvm_reg_block;
  ...
  axi_word_reg words[16];
  axi_ral_backdoor backdoors[16];
  ...
  function void build();
    default_map = create_map("axi", 0, 4, UVM_LITTLE_ENDIAN, 1);
    foreach (words[i]) begin
      words[i] = axi_word_reg::type_id::create($sformatf("word_%0d", i));
      words[i].configure(this);
      words[i].build(32'hc7000000 + 32'(i));
      default_map.add_reg(words[i], uvm_reg_addr_t'(i * 4), "RW");
      backdoors[i] = axi_ral_backdoor::type_id::create($sformatf("backdoor_%0d", i));
      backdoors[i].index = i;
...
class axi_ral_predictor extends uvm_reg_predictor #(axi_item);
  `uvm_component_utils(axi_ral_predictor)
  ...
  virtual function void write(axi_item item);
    ...
    else if (item.response == 0 && !item.cancel) super.write(item);
  endfunction
endclass

class axi_ral_prediction_audit extends uvm_subscriber #(uvm_reg_item);
  `uvm_component_utils(axi_ral_prediction_audit)
  ...
  virtual function void write(uvm_reg_item item);
    if (item.get_status() != UVM_IS_OK || item.get_element_kind() != UVM_REG)
      `uvm_fatal("AXI_RAL_PREDICTOR", "predictor emitted an invalid register operation")
    if (item.get_kind() == UVM_WRITE) writes++;
    else reads++;
  endfunction
endclass

class axi_ral_external_sequence extends axi_sequence;
  `uvm_object_utils(axi_ral_external_sequence)

  function new(string name = "axi_ral_external_sequence");
    super.new(name);
  endfunction

  virtual task body();
    for (int i = 0; i < 16; i++) begin
      transfer(1, 0, 8'(i * 4), 0, 32'h13579bdf ^ (32'(i) * 32'h01010101), 4'(i), i % 3);
      transfer(0, 1, 0, 8'(i * 4));
    end
  endtask
endclass

class axi_ral_helper extends uvm_component;
  `uvm_component_utils(axi_ral_helper)

  axi_reg_block model;
  axi_reg_adapter adapter;
  axi_ral_predictor predictor;
  axi_ral_prediction_audit predictions;
  ...
  virtual function void build_phase(uvm_phase phase);
    ...
    model = axi_reg_block::type_id::create("model");
    model.build();
    adapter = axi_reg_adapter::type_id::create("adapter");
    predictor = axi_ral_predictor::type_id::create("predictor", this);
    predictions = axi_ral_prediction_audit::type_id::create("predictions", this);
  endfunction

  function void attach(axi_agent active_agent, axi_agent passive_agent,
                       axi_scoreboard active_scoreboard, axi_scoreboard passive_scoreboard);
  ...
  function void require_status(uvm_status_e status, string operation);
    if (status != UVM_IS_OK)
      `uvm_fatal("AXI_RAL_STATUS", $sformatf("%s returned %s", operation, status.name()))
  endfunction

  function void require_value(uvm_reg_data_t actual, bit [31:0] expected, string operation);
    if (actual !== uvm_reg_data_t'(expected))
      `uvm_fatal("AXI_RAL_VALUE", $sformatf("%s got=%0h expected=%0h", operation, actual, expected))
  endfunction
  ...
  task require_counts(int before_reads, int before_writes, int expected_reads, int expected_writes,
    ...
    if (predictions.reads - before_reads != expected_reads
      ...
      `uvm_fatal("AXI_RAL_COUNT", $sformatf(
  ...
  task run_checks();
    ...
    axi_ral_external_sequence external_seq;
    uvm_status_e status;
    ...
    if (driver == null || sequencer == null || model.default_map.get_auto_predict())
      `uvm_fatal("AXI_RAL_CONFIG", "RAL transport or explicit prediction is not configured")
    #1;
    initial_reads = predictions.reads;
    initial_writes = predictions.writes;
    initial_driver_completed = (driver.reads + driver.writes);
    reset_hardware();
    ...
    foreach (expected[i])
      require_value(model.words[i].get_mirrored_value(), 32'hc7000000 + 32'(i), "reset mirror");
    ...
    foreach (expected[i]) begin
      ...
      if (!model.words[i].needs_update())
        `uvm_fatal("AXI_RAL_UPDATE", "set did not require a hardware update")
      model.words[i].update(status, UVM_FRONTDOOR);
      require_status(status, "frontdoor update");
      #1;
      if (model.words[i].needs_update())
        `uvm_fatal("AXI_RAL_UPDATE", "completed update did not synchronize the mirror")
    ...
    stage_reads = predictions.reads;
    stage_writes = predictions.writes;
    external_seq = axi_ral_external_sequence::type_id::create("external_seq");
    external_seq.start(sequencer);
    require_counts(stage_reads, stage_writes, 16, 16, "external bus prediction");
    if (external_seq.reads != 16 || external_seq.writes != 16)
      `uvm_fatal("AXI_RAL_COUNT", "external bus sequence did not complete every operation")
    foreach (expected[i]) begin
      bit [31:0] mask = 0;
    ...
    foreach (expected[i])
      require_value(model.words[i].get_mirrored_value(), 32'hc7000000 + 32'(i),
    ...
    foreach (expected[i]) begin
      ...
      if (model.backdoors[i].reads != 2 || model.backdoors[i].writes != 1)
        `uvm_fatal("AXI_RAL_COUNT", "built-in access skipped a register backdoor operation")
    ...
    completed = (driver.reads + driver.writes) - initial_driver_completed;
    if (reads != 1120 || writes != 1152 || completed != 2272 || resets != 3
        || backdoor_reads != 32 || backdoor_writes != 16)
      `uvm_fatal("AXI_RAL_COUNT", "RAL aggregate operation counts are incorrect")
    $display(
        "AXI_RAL_SENTINEL registers=16 reads=%0d writes=%0d resets=%0d backdoor_reads=%0d backdoor_writes=%0d",
