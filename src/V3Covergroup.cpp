...
class FunctionalCoverageVisitor final : public VNVisitor {
    ...
    std::map<AstCoverpoint*, AstVar*> m_convCpVars;  // Converted coverpoint -> VlCoverpoint member
    ...
    void generateConvertedCoverpoint(AstCoverpoint* coverpointp, AstNodeExpr* exprp,
        ...
        m_convCpVars.emplace(coverpointp, cpVarp);
    ...
    void generateCrossCode(AstCoverCross* crossp) {
        ...
        std::vector<AstCoverBin*> expandedBins;
        for (AstCoverpoint* cpp : coverpointRefs) {
            ...
            for (AstNode* binp = cpp->binsp(); binp; binp = binp->nextp()) {
                ...
                // An array declaration contributes one cross dimension entry per bin,
                // matching the counters generated by generateArrayBins().
                bool unsupported = false;
                int index = 0;
                for (AstNodeExpr* const valuep :
                     extractArrayValues(cbinp, cpp->exprp(), unsupported)) {
                    AstCoverBin* const expandedp
                        = new AstCoverBin{cbinp->fileline(),
                                          cbinp->name() + "_" + std::to_string(index++),
                                          valuep,
                                          false,
                                          false,
                                          cbinp->isWildcard()};
                    expandedBins.push_back(expandedp);
                    cpBins.push_back(expandedp);
                }
            }
        ...
        for (AstCoverBin* const binp : expandedBins) pushDeletep(binp);
    }
